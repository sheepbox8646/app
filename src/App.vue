...
async function gen(message: string) {
  try {
    ...
    const engine = await new nc.CarEngine()
      .init('https://cdn.bootcdn.net/ajax/libs/canvaskit-wasm/0.39.1/canvaskit.wasm')
    console.log('engine initialized!')
    let json: string = ''
    if (config.mode === 'server') {
      json = await (await fetch(`${config.backend}/generate`, {
        headers: {
        ...
        body: JSON.stringify({
          ...
          key: sha256(api.value)
        }),
        method: 'POST',
      })).text()
    }
    if (json === 'no') {
      messages.value = '使用码不存在'
      dialogOpen.value = true
      generating.value = false
      return
    }
    if (json === 'runout') {
      messages.value = '使用码已用尽'
      dialogOpen.value = true
      generating.value = false
      return
    }
        ...
        <div class="m-5">
          ...
          <div class="py-3 flex flex-row">
            ...
            <div class="w-1/3 pr-2">
              ...
              <h1 class="font-mono text-sky-300" v-if="couldDownload"><a type="download" :href="download">下载</a>
              </h1>
            </div>
          </div>
          <div class="py-3 flex flex-row">
